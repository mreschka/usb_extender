...
bool USBHostGenericInit ( uint8_t address, uint32_t flags, uint8_t clientDriverID )
{
    ...
    #ifdef USB_GENERIC_SUPPORT_SERIAL_NUMBERS
    {
        ...
        if (USBHostGetStringDescriptor( address, serialNumberIndex, 0, (uint8_t *)gc_DevData.ID.serialNumber, USB_GENERIC_MAX_SERIAL_NUMBER*2, gc_DevData.clientDriverID))
        {
...
bool USBHostGenericEventHandler ( uint8_t address, USB_EVENT event, void *data, uint32_t size )
{
    ...
    case EVENT_TRANSFER:
        if ( (data != NULL) && (size == sizeof(HOST_TRANSFER_DATA)) )
        {
            ...
            if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_IN_EP|gc_DevData.rxEP) )
            {
            ...
            else if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_OUT_EP|gc_DevData.txEP) )
            {
...
uint8_t USBHostGenericRead( uint8_t deviceAddress, uint8_t ep, void *buffer, uint32_t length )
{
    uint8_t RetVal;
    ...
    gc_DevData.rxEP = ep;
    gc_DevData.rxLength = 0;
    RetVal = USBHostRead( deviceAddress, USB_IN_EP|gc_DevData.rxEP, (uint8_t *)buffer, length );
...
void USBHostGenericTasks( void )
{
    ...
    if (gc_DevData.ID.deviceAddress && gc_DevData.flags.initialized)
    {
        ...
        if (gc_DevData.flags.rxBusy)
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_IN_EP|gc_DevData.rxEP, &errorCode, &byteCount ))
            {
                gc_DevData.flags.rxBusy = 0;
        ...
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_OUT_EP|gc_DevData.txEP, &errorCode, &byteCount ))
            {
                gc_DevData.flags.txBusy = 0;
...
uint8_t USBHostGenericWrite( uint8_t deviceAddress, uint8_t ep, void *buffer, uint32_t length )
{
    uint8_t RetVal;
    ...
    gc_DevData.txEP = ep;
    RetVal = USBHostWrite( deviceAddress, USB_OUT_EP|gc_DevData.txEP, (uint8_t *)buffer, length );
    if (RetVal != USB_SUCCESS)
    {
